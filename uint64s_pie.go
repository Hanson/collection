...
import (
	...
	"encoding/json"
	"fmt"
	"github.com/hanson/collection/util"
	"math/rand"
	"sort"
	"strconv"
	"strings"
...
func (ss Uint64s) Diff(against Uint64s) (added, removed Uint64s) {
	...
	diffOneWay := func(ss1, ss2raw Uint64s) (result Uint64s) {
		ss1Map := make(map[uint64]int, 0)
		//这里就自己一个for循环，map不加锁的话快一点
		for _, s := range ss1 {
			ss1Map[s] = 1
		}
		ss2rawMap := make(map[uint64]int, 0)
		for _, element := range ss2raw {
			ss2rawMap[element] = 1
		}
		for _, s := range ss1 {
			if _, ok := ss2rawMap[s]; !ok {
	...
	diffWay := func(ss1 Uint64s, ss2raw map[uint64]int) (result Uint64s) {
		for _, s := range ss1 {
			if _, ok := ss2raw[s]; !ok {
				result = append(result, s)
			}
...
func (ss Uint64s) SortUsing(fn func(a, b uint64) bool) Uint64s {
	// Avoid the allocation. If there is one element or less it is already
	// sorted.
	if len(ss) < 2 {
		return ss
...
// Add items to slice when item not existed
func (ss Uint64s) Add(items ...uint64) (result Uint64s, addedCnt int) {
	result = append(Uint64s{}, ss...)
...
func (ss Uint64s) Remove(items ...uint64) (result Uint64s, removedCnt int) {
	result = Uint64s{}
	for _, v := range ss {
		found := false
		for _, i := range items {
...
func (ss Uint64s) Chunk(chunkSize int, callback func(chunk Uint64s) (stopped bool)) {
	if chunkSize <= 0 {
